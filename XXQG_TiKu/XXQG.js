...
/*
* 导入 Android 提供的操作 SQLite 数据库的类
* 导入 Java 提供的用于创建 HTTP 连接的类
* 导入 Java 提供的用于表示一个 URL 的类
* 导入 Java 提供的用于操作文件的类
*/
...
var url = 'https://ghproxy.com/https://raw.githubusercontent.com/w1770946466/Everyday_News/main/XXQG_TiKu/QuestionBank.db';//题库地址
var path = '/sdcard/QuestionBank.db';//题库手机路径
device.wakeUpIfNeeded();//点亮屏幕
var first = true;//记录是否为答题的第一次
var r;//replace.js文件
var meizhou_txt = hamibot.env.checkbox_02;//每周答题
var zhuanxiang_txt = hamibot.env.checkbox_01;//专项答题
var siren = hamibot.env.checkbox_03;//四人
var shuangren = hamibot.env.shuangren;//双人
var zhanghao = 0;//账号（记录开始第几个账号）
var articles = hamibot.env.article;//文章
var video = hamibot.env.video;//视频
var meiri = hamibot.env.meiri;//每日答题
var tiaozhan = hamibot.env.tiaozhan;//挑战答题
var choose = hamibot.env.select_01;//选择的OCR
var 专项答题下滑 = hamibot.env.select;//专项下滑配置
var 每周答题下滑 = hamibot.env.selectm;//每周下滑配置
var 订阅 = hamibot.env.ssub;//订阅
var 乱序 = 'a';//乱序
var 延迟时间 = hamibot.env.delay_s * 1;//四人双人延迟时间
if (!延迟时间 || 延迟时间 < 0) 延迟时间 = 0;
var stronger = hamibot.env.stronger;//每日答题增强模式
var { username } = hamibot.env;//华为ocr用户名
var { password } = hamibot.env;//华为ocr密码
var { domainname } = hamibot.env;//华为ocr实际账号名
var { projectname } = hamibot.env;//华为ocr项目名
var { endpoint } = hamibot.env;//华为ocr endpoint
var { projectId } = hamibot.env;//华为ocr项目id 
let ocr;//用于加载Auto.js的OCR插件
var token;//华为ocr token
var ttt;//百度ocr token
var question_list = [];//题目、选项和答案作为一个数组
var init_true = false;//是否加载题库
var downloadDialog = null;//显示加载题库对话框
var init_url = 'https://ghproxy.com/https://raw.githubusercontent.com/w1770946466/Everyday_News/main/XXQG_TiKu/question';//题库url
var file_tmp = false;//文件是否下载成功
var tikus = '';//从一个文件中读取题库字符串
var lCount = 1; //挑战答题轮数
...
var myScores = {}; //分数
var article_list = [];//已读文章列表
var delay_time = 1000;//等待时间1000毫秒
var 音字 = false;//判断是否为音字
var o = ['A.', 'B.', 'C.', 'D.', 'AAAA'];
var o1 = ['A', 'B', 'C', 'D', 'AAAA'];
...
function get_baidu_token() {
    ...
    var res = http.post(
        ...
        client_id: hamibot.env.AK.replace(/ /g, ''), // 获取AK并去掉空格
        client_secret: hamibot.env.SK.replace(/ /g, '') // 获取SK并去掉空格
    }
    );
    // 获取token
    ...
    if (shuangren == true || siren == true || 订阅 != 'a' || stronger != 'a' || tiaozhan) {
        ...
        if (tiaozhan && !(siren == true || shuangren == true || 订阅 != 'a' || stronger != 'a')) { }//只开了挑战答题的话
            ...
            threads.start(function () {
                try {
                    var beginBtn;
                    if ((beginBtn = classNameContains("Button").textContains("立即开始").findOne()));
                    else beginBtn = classNameContains("Button").textContains("允许").findOne();
                    beginBtn.click();
                } catch (error) { }
            });
...
function start_app() {
    ...
    console.log("当前手机音量", device.getMusicVolume());
    console.log("正在启动app...");
    ...
    while (!desc("工作").exists() && !id("btn_reg").exists()) {
        console.log("正在进入主页面请稍等！！");
...
function getScores(i) {
        ...
        try {
            className("android.widget.ListView").findOnce().children().forEach(item => {
                ...
                try {
                    ...
                    name = item.child(0).text();//3
                    //name = item.child(3).child(0).text();//3
                    //console.log(name);
                }
                //let str = item.child(2).text().split("/");
                //let score = str[0].match(/[0-9][0-9]*/g);
                let score = item.child(3).child(0).text();//3
                myScores[name] = score;
            });
            err = true;
        } catch (e) {
    ...
    console.log('分享：' + cCount.toString() + '个')
...
function sub() {
    console.show()
    console.info('正在订阅');
    var y = device.height//获取屏幕高
    var a = [7, 5, 4, 3, 2.5, 2.2, 2, 1.8, 1.6, 1.5];
    var x = device.width;//获取屏幕宽
    var asub = 2;
    ...
    for (i = 0; i < a.length; i++) {//i是a中的数
        ...
        while (isfind < 28 && asub > 0) { //isfind往下滑28次数num订阅个数
            ...
            var p = findColor(img, "#E1271A", {
                region: [0.85 * x, 0.1 * y, 1078 - 0.85 * x, 2370 - 0.1 * y],//寻找的范围
                //region: [920, 264, 1078 - 920, 2370 - 264],
                threshold: 20
            });
            if (p) {
                console.info('剩余未订阅：', asub - 1, '个');
                sleep(1000);
                click(p.x + 10, p.y);
                sleep(1000);
                asub--;
                ...
                h1 = (h / 6) * 5;
                h2 = (h / 6);
                //swipe(0.5 * x, 0.6 * y, 0.5 * x, 0.1 * y, 1000);
                swipe(x1, h1, x1, h2, 100);
                //swipe(540, 2160, 613, 2160 - 1200, 1000);
                sleep(1000);
                isfind += 1;
            }
        }
...
function meizhouAnswer() {
    ...
    while (!text("学习积分").exists()) {
        console.info("等待学习积分界面");
        delay(1);
    }
    while (text("学习积分").exists()) {
        text("每周答题").findOne().parent().child(4).click();
        delay(1);
    }
    delay(3);
    var t = 0;
...
function dailyAnswer() {
    ...
    while (true) {
        ...
        } else if (text("排行榜").exists() || desc("工作").exists()) {
            console.log("答题出现错误请手动检查！或者重新运行脚本！");
            back_table();
            break;
        }
    }
...
function challengeQuestion() {
    ...
    while (true) {
        ...
        delay(3);
        if (text('wrong@3x.9ccb997c').exists() || text('结束本局').exists() || text('2kNFBadJuqbAAAAAElFTkSuQmCC').exists() || text("v5IOXn6lQWYTJeqX2eHuNcrPesmSud2JdogYyGnRNxujMT8RS7y43zxY4coWepspQkvw" +
...
function do_contest_answer(depth_option, question1) {
    ...
    console.info(question_list.length)
...
function rand_mode() {
    ...
    function myFunction(num) {
        ...
        console.log("检测到当前在登录页面！！！");
        console.info('开始登录第', zhanghao + 1, "个账号");
        while (text("手机号码").exists()) {
            t = base64.decode(num);
            m = base64.decode(pwd[zhanghao]);
            sleep(1000);
            toast("点击完成");
            sleep(1500);
            setText([0], t);
            ...
            while (!desc("工作").exists()) {
                if (textContains("非常抱歉").exists()) {
                    ...
                    delay(2);
                    swipe(250, 1200, 900, 1200, random(500, 1200));
                    sleep(1000);
                    console.log("首次验证失败，尝试再次验证");
                }
    ...
    question_list = null;
    article_list = null;
    console.log("3s后自动关闭悬浮窗，查看日志请到hamibot内查看。或去/sdcard/脚本/运行日志.txt 目录下查看运行日志");
    delay(3);
    console.hide();
...
function 本地() {
    if (myScores['本地频道'] != 1) {
        ...
        try {
            sleep(3000);
            var first_channel = className("android.widget.LinearLayout").depth(26).drawingOrder(1);
            first_channel.click();
            sleep(1000);
            back();
            sleep(1000);
            console.info("本地频道任务完成！！");
        } catch (error) {
            console.log('本地频道任务执行失败请手动完成！！');
        }
        return
    }
}
 ...
 */
function 每周() {
    if (meizhou_txt == true && meizhou != 0 && zhanghao != 2) {
        console.info('开始每周答题');
        delay(1);
        if (!text("学习积分").exists()) {
            console.info("进入学习积分");
...
function xxjf() {
    ...
    delay(1);
    console.log("点击学习积分");
    text("学习积分").findOne().parent().click();
    delay(1);
}
...
function 四人() {
    if (zsyCount != 0 && siren == true) {
        ...
        while (text("学习积分").exists()) {
            ...
            text("四人赛").findOne().parent().child(4).click();
            delay(1);
        }
        zsyAnswer();
        delay(0.5);
...
function 订() {
    if (订阅 != 'a' && asub != 0) {
        ...
        while (text("学习积分").exists()) {
            text("订阅").findOne().parent().child(4).click();
            delay(1);
        }
        sub(); //订阅
    }
